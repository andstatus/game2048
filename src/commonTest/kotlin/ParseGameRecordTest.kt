...
class ParseGameRecordTest : ViewsForTesting(log = true) {
    ...
    fun parseGameRecordTest() = viewsTest {
        val myContext = MyContext.load(stage)
        testOneString(myContext, gameRecordSingleJson)
        testOneString(myContext, gameRecordConcatenatedJson)
    }

    private suspend fun testOneString(myContext: MyContext, gameRecordString: String) {
        val gameId = History.load(myContext).idForNewGame()
        myContext.storage["game$gameId"] = gameRecordString

        val history = History.load(myContext)
